...
namespace Gravitons.UI.Modal
{
    ...
    public class ModalManager : MonoBehaviour
    {
        ...
        public bool enableMultiModal = false;
        public static bool multiModal;
        ...
        public static Modal Show(string title, string body, ModalButton[] buttons)
        {
            ...
            if (!multiModal)
            {
                if (currentModal) currentModal.gameObject.GetComponent<GenericModal>().Animate(2);
            }
            Modal instance = Instance.InternalShow(Instance.m_DefaultIdentifier, new GenericModalContent() { Title = title, Body = body }, buttons);
